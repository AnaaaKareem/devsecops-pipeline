...
def verify_poc(source_path: str, poc_code: str):
    ...
    try:
        # 🛡️ Use a heredoc-style write to avoid shell quote issues
        # We pass the code as a single string to the container's python
        safe_poc = poc_code.replace("'", "'\\''")
        container.exec_run(f"python3 -c \"with open('poc_exploit.py', 'w') as f: f.write('''{poc_code}''')\"")
